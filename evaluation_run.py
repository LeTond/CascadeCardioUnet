...
class Evaluation(MetaParameters):
    ...
    def nifti_base_evaluation(self, file_name):
    ...
    def nifti_preseg_evaluation(self, file_name):
    ...
    def dicom_base_evaluation(self, file_name):
        create_dir(self.eval_dir)
        neural_model = torch.load(f'{self.project_name}/{self.MODEL_NAME}.pth').to(device=device)
        images, image_shp, def_cord = GetListImages(file_name, self.eval_dir, self.dataset_path, preseg = False).dicom_array(self.KERNEL)
      
        masks_list = PredictionMask(neural_model, self.KERNEL, images, image_shp, def_cord).get_predicted_mask()        
        
        if self.CROPPING:
            DicomSaver(masks_list, file_name, self.eval_dir).save_dicom_mask()
        else:
            DicomSaver(masks_list, file_name, self.eval_dir).save_dicom()

    def dicom_preseg_evaluation(self, file_name):
        create_dir(self.cropp_eval_dir)
        project_name = f'{self.CROPP_EVAL_DIR}/Lr({self.LR})_Drp({self.DROPOUT})_batch({self.BT_SZ})_L2({self.WDC})'
        neural_model = torch.load(f'{project_name}/{self.MODEL_NAME}.pth').to(device=device)        
        images, image_shp, def_cord = GetListImages(file_name, self.eval_dir, self.dataset_path, preseg = True).dicom_array(self.CROPP_KERNEL)

        masks_list = PredictionMask(neural_model, self.CROPP_KERNEL, images, image_shp, def_cord).get_predicted_mask()

        DicomSaver(masks_list, file_name, self.cropp_eval_dir).save_dicom()
